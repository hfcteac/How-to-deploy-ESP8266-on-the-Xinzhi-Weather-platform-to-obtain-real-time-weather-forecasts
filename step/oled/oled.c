...
void OLED_ShownNum(uint8_t x,uint8_t y,int num,uint8_t len,uint8_t size)
{
	if(num<0)
	{
	  OLED_ShowChar(x,y,'-',size);
    OLED_ShowNum(x+8,y,-num,len,size);
	}
	if(num==0||num>0)
	{
...
//显示天气
void OLED_ShowWte(uint8_t x,uint8_t y,uint8_t no)
{      			    
	uint8_t t,adder=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<16;t++)
		{
				OLED_WR_Data(wte[2*no][t]);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_Data(wte[2*no+1][t]);
				adder+=1;
      }					
}
//显示风向
void OLED_Showwind(uint8_t x,uint8_t y,uint8_t no)
{      			    
	uint8_t t,adder=0;
	OLED_Set_Pos(x,y);	
    for(t=0;t<16;t++)
		{
				OLED_WR_Data(loc[2*no][t]);
				adder+=1;
     }	
		OLED_Set_Pos(x,y+1);	
    for(t=0;t<16;t++)
			{	
				OLED_WR_Data(loc[2*no+1][t]);
				adder+=1;
      }					
}
